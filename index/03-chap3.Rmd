...
```{r mcvean, results="asis", echo=FALSE, fig.cap="Coefficients de métissage et ACP", out.width = "300px", fig.align='center'}
...
include_graphics("figure/background.pdf")
```
...
```{r}
im.df <- data.frame(methods = c("RNDmin", "Bdf", "D", "pcadapt"),
                    nb_of_pop = c("3", "4", "4", ">= 3"),
                    outgroup = c("Oui", "Oui", "Oui", "Oui"))
...
p0 <- dplyr::bind_rows(
  ...
  geom_errorbar(aes(ymin = value - se, ymax = value + se), 
                width =.05, size = 1.5) +
  ylim(0, 105) + facet_grid(~stat_f, margins = "am") + ylab("") + xlab(expression(atop(bold("Difference of ancestry between introgressed"), bold("and non-introgressed regions")))) +
