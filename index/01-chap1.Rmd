...
Le taux de fausse découverte, correspond à la proportion de faux positifs 
parmi les positifs. En notant $FP$ le nombre de faux positifs, $FP$ le nombre
de vrais positifs, on définit le taux de fausse découverte $FDR$ par : 
$$ FDR = \mathbb{E}\Big[\frac{FP}{TP + FP} 1_{FP+TP > 0}\Big] $$
- Référence cours de Christophe Giraud

q-value, bonferroni, benjamini-hochberg
La figure suivante donne les comparaisons entre les différentes procédures de 
correction :
...
```{r march3plot, fig.height=3, fig.width=6, fig.cap="$\\beta$"}
